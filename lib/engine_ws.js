...
WSEngine.prototype.step = function (requestSpec, ee) {
  ...
  let f = function(context, callback) {
    ...
    let processFunc = self.config.processor['beforeSend'];
    if (payload && processFunc) {
      payload = processFunc(payload, context, ee);
    }
    ...
    context.ws.send(payload, function(err) {
      if (err) {
        ...
        let processFunc = self.config.processor['onError'];
        if (processFunc) {
          processFunc(payload, err, context, ee);
        }
...
WSEngine.prototype.compile = function (tasks, scenarioSpec, ee) {
  ...
  return function scenario(initialContext, callback) {
    function zero(callback) {
      ...
      let processFunc = config.processor['beforeConnect'];
      if (processFunc) {
        config = processFunc(config, initialContext);
      }
      ...
      ws.on('message', function(data) {
        let processFunc = config.processor['onMessage'];
        if (processFunc) {
          processFunc(data);
        }
      });
